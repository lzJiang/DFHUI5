sap.ui.define([
    ...
    "sap/m/MessageToast",
    "sap/m/MessageBox",
    "zui50005/model/odataUtil",
    "sap/ui/model/Sorter",
    "zui50005/model/formatter",
    ...
    return BaseController.extend("zui50005.controller.mainView", {
        currentType: null,
        confirm: null,
        formatter: formatter,
        onInit() {
            var purchaseorderdatestart = new Date();
            purchaseorderdatestart.setDate(purchaseorderdatestart.getDate() - 7);
            var purchaseorderdatestart = purchaseorderdatestart.toISOString().substring(0, 10);
            var productionplant = sessionStorage.getItem("productionplant");
            this.getView().setModel(new JSONModel({
                purchaseorderdatestart:purchaseorderdatestart,
                cpname:"",
        ...
        _onDataSearch: function () {
            ...
            if (tokens1 && tokens1.length > 0) {
                tokens1.forEach(function (token) {
                    if (purchaseorder == "") {
                        purchaseorder = token.getKey();
                    } else {
                        purchaseorder = purchaseorder + ";" + token.getKey()
                    }
                })
            };
            RequestParameter = {
                productionplant: searchData.productionplant,
                purchaseorderdatestart: this.isEmpty(searchData.purchaseorderdatestart)?"":searchData.purchaseorderdateend,
                purchaseorderdateend:this.isEmpty(searchData.purchaseorderdateend)?"":searchData.purchaseorderdateend,
                purchaseorder: purchaseorder,
                cpname: this.isEmpty(searchData.cpname)?"":searchData.cpname
            };
            var req = that.setReq("PP0011", RequestParameter);
            that.globalBusyOn();
                    ...
                    if (returnitemd && returnitemd.length > 0) {
                        returnitemd.forEach(function (returnitem) {
                            data.forEach(function (dataitem) {
                                if (dataitem.purchaseorder == returnitem.purchaseorder
                                    && dataitem.purchaseorderitem == returnitem.purchaseorderitem
        ...
        checkSave:function (selectedItem) {
            ...
            selectedItem.forEach(function (item) {
                if (!that.isEmpty(item.zllno)) {
                    checkSave.flag = 'E';
                    checkSave.message = '采购订单行' + item.purchaseorder + "-" + item.purchaseorderitem + '预留项目' + item.reservationitem + '已生成领料单，请勿重复处理';
                    return;
                }
            });
            return checkSave;
