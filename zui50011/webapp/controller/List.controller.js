sap.ui.define([
    ...
    return BaseController.extend("zui50011.controller.List", {
        onInit: function () {
            ...
            var downloadurl = "";
            https://my200868.s4hana.sapcloud.cn/sap/opu/odata4/sap/zui_zt_batch_config_o4/srvd/sap/zui_zt_batch_config_o4/0001/config(UUID=21a75e1a-a64d-1eef-b2b3-e30585aacbd5,IsActiveEntity=true)/Template
            if (window.location.host == "my200828.s4hana.sapcloud.cn") {
                ...
                downloadurl = window.location.origin + "/sap/opu/odata4/sap/zui_zt_batch_config_o4/srvd/sap/zui_zt_batch_config_o4/0001/config(UUID=21a75e1a-a64d-1eef-b2b3-e30585aacbd5,IsActiveEntity=true)/Template"
            }
            this.getView().setModel(new JSONModel({
                purchaseorderdatestart: startdatestr
            }), "searchModel");
        ...
        _onDataSearch: function () {
            ...
            var req = that.setReq("PP0019", RequestParameter);
            that.globalBusyOn();
        ...
        _onUpload: function (payload) {
            ...
            for (let index = 0; index < payload.length; index++) {
                ...
                if (payload[index].outbillno.sheetName == "HEADER") {
                    ...
                    head.purchaseordertype = that.getExcelCellData(payload[index].purchaseordertype);
                    head.purchaseorderdate = that.getExcelCellData(payload[index].purchaseorderdate);
                    head.supplier = that.getExcelCellData(payload[index].supplier);
                    head.purchasingorganization = that.getExcelCellData(payload[index].purchasingorganization);
                    head.purchasinggroup = that.getExcelCellData(payload[index].purchasinggroup);
                    head.item = [];
                    data.push(head)
                }
            }
            ...
            if (data && data.length > 0) {
                data.forEach(function (head) {
                    for (let index = 0; index < payload.length; index++) {
                        ...
                        if (payload[index].outbillno.sheetName == "ITEM"
                            && payload[index].outbillno.formattedValue == head.outbillno) {
                            let item = {};
                            item.outbillno = that.getExcelCellData(payload[index].outbillno);
                            item.outbillitemno = that.getExcelCellData(payload[index].outbillitemno);
                            item.purchaseorderitemtext = that.getExcelCellData(payload[index].purchaseorderitemtext);
                            item.plant = that.getExcelCellData(payload[index].plant);
                            item.netpriceamount = that.getExcelCellData(payload[index].netpriceamount);
                            item.orderquantity = that.getExcelCellData(payload[index].orderquantity);
                            item.purchaseorderquantityunit = that.getExcelCellData(payload[index].purchaseorderquantityunit);
                            item.purchaseorderitemcategory = that.getExcelCellData(payload[index].purchaseorderitemcategory);
                            item.material = that.getExcelCellData(payload[index].material);
                            item.taxcode = that.getExcelCellData(payload[index].taxcode);
                            item.schedulelinedeliverydate = that.getExcelCellData(payload[index].schedulelinedeliverydate);
                            item.masterfixedasset = that.getExcelCellData(payload[index].masterfixedasset);
                            item.subcontractor = that.getExcelCellData(payload[index].subcontractor);
                            item.supplierissubcontractor = that.getExcelCellData(payload[index].supplierissubcontractor);
                            head.item.push(item)
                        }
                    }
                });
        ...
        _onSave: async function () {
            ...
            for (let index = 0; index < selectItem.length; index++) {
                ...
                var req = that.setReq("PP0020", RequestParameter);
                that.globalBusyOn();
                        ...
                        if (returndata) {
                            data.forEach(function (dataitem) {
                                if (dataitem.outbillno == returndata.outbillno) {
                                    ...
                                    dataitem.purchaseorder = returndata.purchaseorder;
                                    if (dataitem.flag == 'S') {
