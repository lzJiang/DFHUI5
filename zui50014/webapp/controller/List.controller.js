sap.ui.define([
    ...
    return BaseController.extend("zui50014.controller.List", {
        onInit: function () {
            ...
            if (window.location.host == "my200828.s4hana.sapcloud.cn") {
                downloadurl = window.location.origin + "/sap/opu/odata4/sap/zui_zt_batch_config_o4/srvd/sap/zui_zt_batch_config_o4/0001/config(UUID=ff065470-9146-1edf-b3cd-1a8256026ea2,IsActiveEntity=true)/Template"
                ...
                downloadurl = window.location.origin + "/sap/opu/odata4/sap/zui_zt_batch_config_o4/srvd/sap/zui_zt_batch_config_o4/0001/config(UUID=21a75e1a-a64d-1eef-b3cd-250871230be1,IsActiveEntity=true)/Template"
            }
        ...
        _onDataSearch: function () {
            ...
            var req = that.setReq("MM0003", RequestParameter);
            that.globalBusyOn();
        ...
        _onUpload: function (payload) {
            ...
            for (let index = 0; index < payload.length; index++) {
                ...
                if (!this.isEmpty(payload[index].outbillno) && payload[index].outbillno.sheetName == "HEADER") {
                    let head = {};
                ...
                data.forEach(function (head) {
                    for (let index = 0; index < payload.length; index++) {
                        ...
                        if (payload[index].outbillno.sheetName == "ITEM"
                            ...
                            item.goodsmovementtype = that.getExcelCellData(payload[index].goodsmovementtype);
                            ...
                            item.storagelocation = that.getExcelCellData(payload[index].storagelocation);
                            item.batchwms = that.getExcelCellData(payload[index].batchwms);
                            item.manufacturedate = that.getExcelCellData(payload[index].manufacturedate);
                            item.entryunit = that.getExcelCellData(payload[index].entryunit);
                            item.quantityinentryunit = that.getExcelCellData(payload[index].quantityinentryunit);
                            item.issuingorreceivingstorageloc = that.getExcelCellData(payload[index].issuingorreceivingstorageloc);
                            item.zvension = that.getExcelCellData(payload[index].zvension);
                            item.zzlast = that.getExcelCellData(payload[index].zzlast);
                            if (that.isEmpty(head.plant)) {
                                head.plant = item.plant
                            }
                            if (that.isEmpty(head.goodsmovementtype)) {
                                head.goodsmovementtype = item.goodsmovementtype
                            }
                            head.item.push(item)
                        }
                    }
        ...
        _onSave: async function () {
            ...
            for (let index = 0; index < selectItem.length; index++) {
                ...
                var req = that.setReq("MM0004", RequestParameter);
                that.globalBusyOn();
                        ...
                        if (returndata) {
                            data.forEach(function (dataitem) {
                                if (dataitem.outbillno == returndata.outbillno) {
                                    ...
                                    dataitem.materialdocument = returndata.materialdocument;
